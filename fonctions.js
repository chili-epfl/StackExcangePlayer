...
function cleanCurrentContent(CurrentContent) {
  ...
  for (i = 0; i < Object.size(result); i++) {
    if (
      Object.size(result[i]) == 0 ||
      result[i].match(/\d\d:\d\d:\d\d.\d\d\d --> \d\d:\d\d:\d\d.\d\d/)
    ) {
      result.splice(i, 1);
...
function FindTime(time, TIMEArray, passage) {
  for (j = 0; j < Object.size(TIMEArray); j++) {
    if (
      (time >= TIMEArray[j].min && time <= TIMEArray[j].max) ||
      time <= TIMEArray[j].min
    ) {
      if (passage == 2) {
        j++;
      }
...
function diri() {
  ...
  SendActivity(choix + 4, player.currentTime() + ';' + new Date().getTime()); // 5 is an offset for the db
  if (configHelp == 1) {
    overlay[0]['start'] = 0;
...
function display(index) {
  ...
  SendActivity(
    'click url',
    player.currentTime() + ';' + ';' + new Date().getTime() + ';' + url
  );
}
